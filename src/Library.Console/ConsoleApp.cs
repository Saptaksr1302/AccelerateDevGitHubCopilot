...
public class ConsoleApp
{
    ...
    async Task<List<Book>> LoadBooks()
    {
        await _jsonData.EnsureDataLoaded();
        return _jsonData.Books!;
    }

    async Task<Loan?> GetLoanByBookId(int bookId)
    {
        await _jsonData.EnsureDataLoaded();
        return _jsonData.Loans!.FirstOrDefault(l => l.BookItem!.BookId == bookId && l.ReturnDate == null);
    }
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        List<Book> books = await _jsonData.LoadBooks();

        // Find the book with the matching title
        Book? matchingBook = books.FirstOrDefault(b => b.Title.Equals(bookTitle, StringComparison.OrdinalIgnoreCase));
        ...
        Loan? loan = await _jsonData.GetLoanByBookId(matchingBook.Id);
        ...
        {
            Console.WriteLine($"{matchingBook.Title} is on loan to another patron. The return due date is {loan.DueDate}.");
        }
