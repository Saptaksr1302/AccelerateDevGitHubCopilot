...
namespace Library.ApplicationCore.Enums
{
    public static class EnumHelper
    {
        ...
        public static string GetDescription(Enum value)
        {
            ...
            {
                if (LoanExtensionStatusDescriptions.TryGetValue(loanExtensionStatus, out var description))
                {
                    return description;
                }
                return loanExtensionStatus.ToString();
            }

            if (value is LoanReturnStatus loanReturnStatus)
            {
                if (LoanReturnStatusDescriptions.TryGetValue(loanReturnStatus, out var description))
                {
                    return description;
                }
                return loanReturnStatus.ToString();
            }

            if (value is MembershipRenewalStatus membershipRenewalStatus)
            {
                if (MembershipRenewalStatusDescriptions.TryGetValue(membershipRenewalStatus, out var description))
                {
                    return description;
                }
                return membershipRenewalStatus.ToString();
            }
